module.exports = (app, { mainApi, requireAdmin, requireLogin }) => {
  ...
  app.get('/admin/rentals', requireAdmin, async (req, res, next) => {
    try {
      ...
      if (!result.ok) {
        req.flash('error', (result.body && result.body.error) || 'โหลดข้อมูลร้านเช่าไม่สำเร็จ กรุณาลองใหม่');
        return res.render('admin-rentals', {
          rentedShops: [], featureCatalog: [], featureReleases: [], sales: [], transactions: [], discordSettings: {},
          discordConfigured: false, discordReady: false, title: 'ร้านเช่าและประวัติ',
        });
      }
  ...
  app.post('/admin/rentals/features', requireAdmin, async (req, res, next) => {
    try {
      const result = await mainApi.updateRentalFeatures({
        scope: req.body.scope,
        shopIds: [].concat(req.body.shopIds || []),
        feature: req.body.feature,
        action: req.body.action,
      });
      req.flash(result.ok ? 'success' : 'error', result.ok
        ? `แก้ไขฟีเจอร์สำเร็จ ${result.body.updatedCount} ร้าน`
        : (result.body.error || 'แก้ไขฟีเจอร์ไม่สำเร็จ'));
      res.redirect('/admin/rentals');
    } catch (error) { next(error); }
  });
  app.post('/admin/rentals/releases', requireAdmin, async (req, res, next) => {
    try {
      const result = await mainApi.createFeatureRelease(req.body);
      req.flash(result.ok ? 'success' : 'error', result.ok ? 'เพิ่มแพ็กเกจระบบแล้ว พร้อมเลือกนำส่งให้ลูกค้า' : (result.body.error || 'เพิ่มแพ็กเกจไม่สำเร็จ'));
      res.redirect('/admin/rentals');
    } catch (error) { next(error); }
  });
  app.post('/admin/rentals/releases/deploy', requireAdmin, async (req, res, next) => {
    try {
      const result = await mainApi.deployFeatureRelease({ releaseId: req.body.releaseId, scope: req.body.scope, shopIds: [].concat(req.body.shopIds || []) });
      req.flash(result.ok ? 'success' : 'error', result.ok ? `นำส่ง ${result.body.releaseName} รุ่น ${result.body.version} สำเร็จ ${result.body.updatedCount} ร้าน` : (result.body.error || 'นำส่งระบบไม่สำเร็จ'));
      res.redirect('/admin/rentals');
    } catch (error) { next(error); }
  });
  ...
  app.get('/sales', requireLogin, async (req, res, next) => {
    try {
      ...
      if (!result.ok) {
        req.flash('error', (result.body && result.body.error) || 'โหลดประวัติไม่สำเร็จ กรุณาลองใหม่');
        return res.render('sales', { title: 'ประวัติเช่าเดิม', sales: [] });
      }
