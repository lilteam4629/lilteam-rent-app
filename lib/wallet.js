...
async function recordTrueMoneyResult(userId, voucherCode, amount, senderName = '') {
  const numericAmount = Number(amount);
  if (!Number.isFinite(numericAmount) || numericAmount <= 0) throw new Error('จำนวนเงินจากซองไม่ถูกต้อง');
  return transactRetry(data => {
    ensureData(data);
...
async function redeem(userId, input) {
  ...
  try {
    ...
    try {
      const recorded = await recordTrueMoneyResult(userId, voucherCode, result.amount, result.senderName);
