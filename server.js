...
async function importLegacyPaymentOnce(){
  const p=cloudStore.payment();if(Number(p.legacyPaymentImportVersion||0)>=3)return;
  ...
  delete p.byshopApiKey;delete p.byshopEndpoint;p.truemoneyEnabled=result.body.payment.truemoneyEnabled===true;p.truemoneyImported=true;p.legacyPaymentImported=true;p.legacyPaymentImportVersion=3;cloudStore.save();
}
